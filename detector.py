...
from utils.log import excel_logs
...
class Detector(object):
    ...
    def train(self):
        ...
        for i_epoch in range(epoch):
            ...
            r_loss_l = []
            p_loss_l = []
            for x, y,labels,node_edge_idx,res_edge_idx in dataloader:
                _start = time.time()
                ...
                p_loss = loss_fn(p_out,r_out, y,x)
                loss = p_loss[0]
                loss.backward()
                optimizer.step()

                #r_loss_l.append(r_loss.item())
                p_loss_l.append(p_loss[0].item())
                acu_loss += loss.item()
    ...
    def test(self):
        ...
        mse_loss_list=[]
        mae_loss_list=[]
        for i,(x, y,labels,node_edge_index,res_edge_index) in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                mse_loss_list.append(p_loss[0])
                mae_loss_list.append(p_loss[1])
        mse_loss = np.mean(mse_loss_list)
        mae_loss = np.mean(mae_loss_list)
        sup_para = df({'epoch':[self.num_epochs],'lr':[self.lr],'dp':[self.dropout],'bs':[self.batch_size],'dataset':[self.dataset],
        'win_s':[self.win_size],'predict_len':[self.predict_len],'emb_s':[self.emb_size]})
        performance = df({'mse':[mse_loss],'mae':[mae_loss]})
        excel_logs('log_single_map.xlsx',sup_para,performance)
